...
def part1(file):
    # This is my original answer. It never actually terminates!
    ...
    while 1:
        ...
        if max(x for x, y in stars) - min(x for x, y in stars) < 100:
            print_board(stars)
            print()
...
def part2(file):
    ...
    for i in itertools.count(1):
        ...
        if max(x for x, y in stars) - min(x for x, y in stars) <= len(myline):
            print_board(stars)
            return i
