...
def part1(file):
    ...
    for line in file:
        if "Guard" in line:
            y, m, d, h, mn, guard = get_digits(line)
            on_duty = guard
        elif "asleep" in line:
            y, m, d, h, sleep = get_digits(line)
        elif "wakes" in line:
            y, m, d, h, now = get_digits(line)
            time[guard] += now - sleep

    id_, asleep = max(time.items(), key=lambda x: x[1])
    ...
    for line in file:
        if "Guard" in line:
            ...
            y, m, d, h, now = get_digits(line)
            if on_duty == id_:
                for i in range(sleep, now):
                    minutes[guard][i] += 1

    ((key, value),) = minutes[id_].most_common(1)
    return id_ * key
...
def part2(file):
    ...
    (((id_, mn), value),) = minutes.most_common(1)
    return id_ * mn
